...
echo "Installation directory for sources and executables:\
  path_devel=$path_devel"
echo "Installation directory for links to executables:\
  path_sync=$path_sync"

# Set Fortran compiler
setenv FORTRAN gfortran

# Add flags for debugging (gfortran)
setenv FORTRAN "$FORTRAN -g -fbacktrace -ffpe-trap=invalid,zero,overflow -fbounds-check -fcheck-array-temporaries"

# Some programs use uninitialized variables
# Programs known tho have this problem: newtime (integer only), ...?
# Better force gfortran to initialize all of them (like ifort and pgf90 do):
setenv FORTRAN "$FORTRAN -finit-real=zero -finit-integer=0"
# (does not help with caltra bug)

# ioinp_cdo.f has two statements ('maxps = -1.e39' and 'minps =
# 1.e39') that indicate that this code needs double precision by
# default. This might collide with the netcdf library.
#setenv FORTRAN "$FORTRAN -fdefault-real-8"
# (does not help with caltra bug)
